...
install_dependencies() {
    ...
    if [ ! -f "$venv_dir/bin/activate" ]; then
        ...
        python3 -m venv "$venv_dir" >> "$SHOESTRING_DIR/setup.log" 2>&1 || error_exit "仮想環境の作成に失敗: $venv_dir。権限を確認してね: ls -ld $SHOESTRING_DIR"
...
extract_main_account() {
    ...
    if [ "$ENCRYPTED" = true ]; then
        ...
        printf "%s" "$SYMBOL_BOOTSTRAP_PASSWORD" | symbol-bootstrap decrypt --source "$yml_file" --destination "$decrypted_yml" || error_exit "addresses.yml の復号に失敗"
...
EOF
    ...
    python3 -m shoestring init "$config_file" --package "$network_type" > "$SHOESTRING_DIR/install_shoestring.log" 2>&1 || {
...
EOF
    ...
    if ! $SKIP_CONFIRM; then
        ...
        cat "$overrides_file"
        if ! confirm "この設定で大丈夫？"; then
            error_exit "overrides.ini を手動で修正して再実行してね: nano $overrides_file"
        fi
    fi
    ...
    if [ -f "$src_harvesting" ]; then
        cp "$src_harvesting" "$shoestring_subdir/" || error_exit "config-harvesting.properties のコピーに失敗"
    ...
    python3 -m shoestring import-bootstrap --config "$config_file" --bootstrap "$BOOTSTRAP_DIR" > "$SHOESTRING_DIR/import_bootstrap.log" 2>&1 || {
        log "import-bootstrap エラー: $(cat "$SHOESTRING_DIR/import_bootstrap.log")" "ERROR"
        print_warning "import-bootstrap に失敗したけど、ファイルはコピー済みだから続行するよ"
    }
    
    # CA秘密鍵のパス
    local ca_key_path="$SHOESTRING_RESOURCES/ca.key.pem"
...
show_post_migration_guide() {
    ...
    echo
    print_info "大事なファイル："
    echo "  🔑 CA秘密鍵: $SHOESTRING_RESOURCES/ca.key.pem"
...
main() {
    ...
    if [ -f "$ca_key_pem" ]; then
        ...
        print_info "ca.key.pem を生成したよ: $ca_key_pem"
    else

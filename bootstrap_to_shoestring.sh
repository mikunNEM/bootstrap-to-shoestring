...
install_dependencies() {
    ...
    if [[ $os_name == "ubuntu" || $os_name == "debian" ]]; then
     print_info "APT チェックとアップデート..."
        ...
        retry_command "sudo apt-get install -y python3-apt libapt-pkg-dev software-properties-common" || {
            error_exit "python3-apt または software-properties-common のインストールに失敗しました。手動でインストールしてください: sudo apt-get install -y python3-apt software-properties-common"
        }
        # apt_pkg の動作確認
